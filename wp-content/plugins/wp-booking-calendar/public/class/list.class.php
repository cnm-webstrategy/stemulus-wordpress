...
class wp_booking_calendar_public_lists {	
	...
	public function getReservationsList($filter,$order_by,$calendar_id) {
		...
		$reservationsQry = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation r INNER JOIN booking_slots s ON s.slot_id=r.slot_id WHERE r.calendar_id = %d AND s.calendar_id = %d ".$filter." ".$order_by,$calendar_id,$calendar_id));
	...
	public function getSlotsPerDay($year,$month,$daynum, $calendar_id,$bookingSettingObj) {
		...
		if($year."-".$month."-".$daynum == date('Y-m-d')) {
			$slotsQry = $wpdb->get_results($wpdb->prepare("SELECT SUM(s.slot_av) AS av_seats,s.* FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots s WHERE s.slot_active=%d AND s.slot_date = %s  AND REPLACE(s.slot_time_from,':','') >= DATE_FORMAT(NOW(),'%H%i%s') AND s.calendar_id=%d GROUP BY s.slot_id",1,$year."-".$month."-".$daynum,$calendar_id));
		} else {
	...
	public function getSlotsPerDayList($year,$month,$day,$calendar_id,$bookingSettingObj) {
		...
		if($year."-".$month."-".$day == date('Y-m-d')) {
			...
			$slotsQry = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots WHERE slot_active=%d AND slot_date = %s AND REPLACE(slot_time_from,':','') >= DATE_FORMAT(NOW(),'%H%i%s') AND calendar_id=%d ORDER BY slot_time_from",1,$year."-".$month."-".$day,$calendar_id));
		} else {
