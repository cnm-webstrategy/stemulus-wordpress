...
function booking_calendar_install_db($blog_prefix = '') {
	...
	INSERT INTO ".$wpdb->base_prefix.$blog_prefix."booking_texts (page_id, text_label, text_value)
	SELECT * FROM (SELECT '2','CONFIGURATION_EMAIL_FROM_RESERVATION_SUBLABEL' as label, 'E-mail address shown in the field \"From\" in every e-mail sent to confirm the reservation to your customer') AS tmp
	WHERE NOT EXISTS (
...
') AS tmp
	...
	WHERE NOT EXISTS (
		SELECT text_label FROM ".$wpdb->base_prefix.$blog_prefix."booking_texts WHERE page_id = 6 AND text_label = 'RESERVATION_SEARCH_FILTER_WP_USER_CHOOSE'
	) LIMIT 1;
...
function wp_booking_calendar_process_update(){
	...
	switch($current_version) {
		case '1.0.0':
			...
			update_option('wbc_version','4.0.9');
