...
if (!class_exists('tt_gfonts')) {
    ...
    class tt_gfonts {
        ...
        // function tt_gfonts(){$this->__construct();} // PHP 4
        ...
        function __construct(){
			...
			$this->tags = array(
					...
					/*'h4'				=> __( 'Header 4 (h4 - rarely used)', $this->localizationDomain),
					'h5'				=> __( 'Header 5 (h5 - rarely used)', $this->localizationDomain),
					'h6'				=> __( 'Header 6 (h6 - rarely used)', $this->localizationDomain),*/
		...
		function tt_gfp_customizer_head() {
			?>
			<link rel="stylesheet" type="text/css" href="<?php echo plugins_url( 'css/tt_gfp.css' , __FILE__ ); ?>">
		...
		function tt_gfp_register_customizer_options( $wp_customize ) {
			...
			$wp_customize->add_section( 'tt_gfp_customizer_section_fonts', array(
				'title'    => __( 'Google Fonts for ThemeTrust', 'themetrust' ),
				'description'    => __( 'To edit the font for a specific tag, check the box beside and begin typing the font family you would like. The box should auto-complete with the available fonts.', 'themetrust' ),
				'priority' => 1
			));
			...
			foreach ($tags_local as $key => $value) {
				...
				$wp_customize->add_setting( 'tt_gfp_' . $key . '_tag');
				$wp_customize->add_control( new TT_GFP_Tag( $wp_customize, 'tt_gfp_' . $key . '_tag', array(
					'label'		=> __( $value, 'themetrust' ),
					'section'   => 'tt_gfp_customizer_section_fonts',
					'settings'  => 'tt_gfp_' . $key . '_tag',
					'priority' => "1".$i++,
				) ) );

				// CSS Selectors

				$wp_customize->add_setting( 'tt_gfp_' . $key . '_css_selectors', array(
					'default' => '',
					'transport' => 'refresh'
				));

				$wp_customize->add_control( 'tt_gfp_' . $key . '_css_selectors',
				array(
					'type'	   => 'text',
					'label'    => __( 'CSS Selectors to Target (optional)', 'themetrust' ),
					'section'  => 'tt_gfp_customizer_section_fonts',
					'settings' => 'tt_gfp_' . $key . '_css_selectors',
					'priority' => "1".$i++,
				) );

				// Font Family

				$wp_customize->add_setting( 'tt_gfp_' . $key . '_font_family', array(
					'default' => 'default',
					'transport' => 'refresh'
				));

				$wp_customize->add_control( 'tt_gfp_' . $key . '_font_family',
				array(
					'type'	   => 'select',
					'label'    => __( 'Font Family', 'themetrust' ),
					'section'  => 'tt_gfp_customizer_section_fonts',
					'settings' => 'tt_gfp_' . $key . '_font_family',
					'priority' => "1".$i++,
				...
				$wp_customize->add_setting( 'tt_gfp_' . $key . '_font_weight', array(
					'default' => 'regular',
					'transport' => 'refresh'
				));

				$wp_customize->add_control( 'tt_gfp_' . $key . '_font_weight', array(
					'type'     => 'select',
					'label'    => __( 'Weight', 'themetrust' ),
					'section'  => 'tt_gfp_customizer_section_fonts',
					'priority' => "1".$i++,
		...
		function tt_gfp_generate_styles() {
			...
			foreach($tags_local as $key => $value) {
				$font_family_temp = get_theme_mod("tt_gfp_" . $key . "_font_family");
				$font_weight_temp = get_theme_mod("tt_gfp_" . $key . "_font_weight");
				$font_family = str_replace(" ", "+", $font_family_temp);
				$css_selectors_string = preg_replace('/[^a-z0-9 \#\.\,\-]/i', '', strip_tags(get_theme_mod("tt_gfp_" . $key . "_css_selectors"))) ;
