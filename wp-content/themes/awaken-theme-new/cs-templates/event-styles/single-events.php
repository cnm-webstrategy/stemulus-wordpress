	...
	if (have_posts()):
		...
		$post_xml = get_post_meta($post->ID, "events", true);
			...
			if ($post_xml <> "") {
					...
					$cs_post_event_start_time = $cs_xmlObject->dynamic_post_event_start_time+0;
					...
					$cs_post_event_from_date = $cs_xmlObject->dynamic_post_event_from_date+0;
		...
		<div class="col-md-8">
                          ...
                          <div class="post-option-panel">
                            ...
                            <div class="event-info">
                            	<?php if(isset($time_formate) and $time_formate <> '' and $time_formate == "12 hour"){ ?>
                            
                                <ul>
								<?php if($cs_post_event_start_time<>''){?>
                                    <li><?php _e('Event Time','Awaken');?>
                                        <span><i class="fa fa-clock-o"></i><?php echo date(get_option('time_format'),strtotime($cs_post_event_start_time+0)) ?><?php echo ($cs_post_event_end_time<>'')?'-'.date(get_option('time_format'),strtotime($cs_post_event_end_time+0)):''; ?></span>
                                    </li>
									<?php 
										}
									if($cs_post_event_from_date<>''){?>
                                    <li><?php _e('Event Start Date','Awaken');?>
                                        <span><i class="fa fa-clock-o"></i><?php echo date(get_option('date_format'),strtotime($cs_post_event_from_date+0)) ?></span>
                                    </li>
									<?php 
										}
                               ...
                               <?php }else if(isset($time_formate) and $time_formate <> '' and $time_formate == "24 hour"){?>
                               <ul>
								<?php if($cs_post_event_start_time<>''){?>
                                    <li><?php _e('Event Time','Awaken');?>
                                        <span><i class="fa fa-clock-o"></i>
										<?php 
										$newdate =  date('M j, Y @ G:i',strtotime($cs_post_event_start_time+0));
										$myDate =  explode("@",$newdate);
										echo $myDate[1]." - ";
										?><?php 
										$newdate1 = ($cs_post_event_end_time<>'')?'-'.date('M j, Y @ G:i',strtotime($cs_post_event_end_time+0)):''; 
										$newDate1 = explode("@",$newdate1);
										echo $newDate1[1];
										?></span>
                                    </li>
									<?php 
										...
										//	$username  			= $user_info->display_name;
											...
											$html  .='<li>';
												if(isset($image_url) && $image_url <> ''){
												  ...
												  $html .='<div class="text">
																<a href="'.get_author_posts_url($user_id).'">'.get_the_author().'</a>
				 ...
				 if($custom_query->have_posts()){
						 ...
						 while ($custom_query->have_posts()) : $custom_query->the_post();
							...
							if ( $post_xml <> "" ) {
									...
									$cs_post_event_start_time = strtotime($cs_xmlObject->dynamic_post_event_start_time+0);
									$cs_post_event_end_time = strtotime($cs_xmlObject->dynamic_post_event_end_time);
									$cs_post_event_from_date = strtotime($cs_xmlObject->dynamic_post_event_from_date+0);
									$cs_post_location_address = $cs_xmlObject->dynamic_post_location_address;
									...
									<article class="cs-events events-minimal minimal-grid">
										<div class="left-sp">
											...
											<div class="location-info">
												<time datetime="<?php echo esc_attr( $cs_post_event_from_date );?>"> <?php echo date_i18n('M',$cs_post_event_from_date+0); ?> <span><?php echo date_i18n('d',$cs_post_event_from_date+0); ?> </span> </time>
												<div class="info">
													<div class="time-period">
													<time datetime="<?php echo date('d-m-y',$cs_post_event_from_date+0); ?>"><i class="fa fa-clock-o"></i><?php echo date_i18n(get_option('time_format'),$cs_post_event_start_time+0) ?><?php echo ($cs_post_event_end_time<>'')?'-'.date_i18n(get_option('time_format'),$cs_post_event_end_time):''; ?></time> 
													</div>
