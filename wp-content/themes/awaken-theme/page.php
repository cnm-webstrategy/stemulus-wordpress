...
if ( empty($cs_page_bulider) ) {
	$cs_sidebarLayout = $cs_theme_options['cs_default_page_layout'];
	if ( isset($cs_sidebarLayout) && $cs_sidebarLayout == "sidebar_left" ) {
		$cs_sidebarLayout = 'left';
		$page_element_size = "page-content";
		$cs_page_sidebar_left = $cs_theme_options['cs_default_layout_sidebar'];
		$pageSidebar = true;
	} else if ( isset($cs_sidebarLayout) && $cs_sidebarLayout == "sidebar_right" ) {
		$cs_sidebarLayout = 'right';
		$page_element_size = "page-content";
		$cs_page_sidebar_right = $cs_theme_options['cs_default_layout_sidebar'];
		$pageSidebar = true;
	} else {
		$page_element_size = "page-content-fullwidth";
	}
...
if ( isset($cs_xmlObject) && count($cs_xmlObject) > 1 ) {
	...
	if ( $cs_sidebarLayout <> '' and $cs_sidebarLayout <> "none" ) {
		$fullwith_style = 'style="width:100%;"';
		$section_container_style_elements = ' width: 100%;';
		echo '<div class="container">';
		echo '<div class="row">';
		if ( $cs_sidebarLayout <> '' and $cs_sidebarLayout <> "none" and $cs_sidebarLayout == 'left' ) :
			?>
	...
	if ( post_password_required() ) {
		...
		if ( get_the_content() <> '' || $image_url <> '' ) {
			...
			if ( isset($image_url) && $image_url != '' ) {
				?>
				<a rel="prettyPhoto" data-rel="prettyPhoto" href="<?php echo esc_url($image_url); ?>" data-title="">
					<figure>
		...
		foreach ( $cs_xmlObject->column_container as $column_container ) {
			...
			if ( isset($column_container) ) {
				...
				if ( isset($cs_section_background_option) && $cs_section_background_option == 'section-custom-background-image' ) {
					...
					$section_style_elements .= 'background: ' . $cs_section_bg_imageg . ' ' . $cs_section_bg_color . ';' . 'background-size: cover';
				...
				$page_element_size = 'section-fullwidth col-md-12';
	...
	if ( $cs_sidebarLayout <> '' and $cs_sidebarLayout <> "none" and $cs_sidebarLayout == 'right' ) :
		?>
		<aside class="page-sidebar">
