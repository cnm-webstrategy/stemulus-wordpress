            ...
            if (have_posts()):
                while (have_posts()) : the_post();
                    ...
                    if ($post_xml <> "") {
                        ...
                        $cs_post_event_from_date = get_post_meta(get_the_id(), "dynamic_post_event_from_date", true);
                                    ...
                                    <?php if ($cs_post_event_start_time <> '') { ?>
                                            <li><?php _e('Event Time', 'Awaken'); ?>
                                                <span><i class="fa fa-clock-o"></i>
													<?php if($cs_time_formate=='24 hour') {
														echo $cs_post_event_start_time; 
													}
													else{
														$time_in_12_hour_format  = date("g:i a", strtotime($cs_post_event_start_time));
													...
													<?php if ($cs_post_event_end_time <> '') { 
														echo ' - ';
														if($cs_time_formate=='24 hour') {
															echo $cs_post_event_end_time;
														}
														else{
															$time_in_12_hour_format  = date("g:i a", strtotime($cs_post_event_end_time));
															echo $time_in_12_hour_format;
															}
													}
														?>
            ...
            if (isset($categories_list) and $categories_list <> '') {
                ?>
                                ...
                                while ($custom_query->have_posts()) : $custom_query->the_post();
                                    ...
                                    <div class="col-md-4">
                                        <article class="cs-events events-minimal minimal-grid">
                                            <div class="left-sp">
                                                <h2><a href="<?php the_permalink(); ?>"><?php echo substr($title, 0, 30);
                                    echo (strlen($title) > 25) ? '...' : ''; ?></a></h2>
                    ...
                    <?php if ($cs_post_location_address <> '') { ?>
                                                            <span><i class="fa fa-map-marker"></i><?php echo substr($cs_post_location_address, 0, 25);
                        echo (strlen($cs_post_location_address) > 25) ? '...' : ''; ?></span>
                                    <?php } ?>
                                                    </div>
                                                </div>
                                            </div>
