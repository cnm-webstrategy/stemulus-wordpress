...
if (!function_exists('cs_event_listing')) {
    ...
    function cs_event_listing($atts, $content = "") {
        ...
        if ($cs_events_listing_type == "upcoming-events")
            $meta_compare = ">=";
        else if ($cs_events_listing_type == "past-events")
            $meta_compare = "<";
        $cs_counter_events = 0;
        ...
        if ($custom_query->have_posts() <> "") {
            ?>
            <?php
            if (isset($cs_events_section_title) && $cs_events_section_title <> '') {
            ...
            if ($events_pagination == "Show Pagination" and $count_post > $cs_events_num_post and $cs_events_num_post > 0) {
                if (isset($_GET['organizer']))
                    $qrystr .= "&amp;organizer=" . $_GET['organizer'];
                if (isset($_GET['sort']))
...
if (!function_exists('cs_location_fields')) {
    ...
    function cs_location_fields() {
        ...
        if (isset($cs_xmlObject)) {
            if (isset($cs_xmlObject->dynamic_post_location_latitude)) {
                $dynamic_post_location_latitude = $cs_xmlObject->dynamic_post_location_latitude;
            } else {
                $dynamic_post_location_latitude = '';
            }
            if (isset($cs_xmlObject->dynamic_post_location_longitude)) {
                $dynamic_post_location_longitude = $cs_xmlObject->dynamic_post_location_longitude;
            } else {
                $dynamic_post_location_longitude = '';
            }
            if (isset($cs_xmlObject->dynamic_post_location_zoom)) {
                $dynamic_post_location_zoom = $cs_xmlObject->dynamic_post_location_zoom;
            } else {
                $dynamic_post_location_zoom = '';
            }
            if (isset($cs_xmlObject->dynamic_post_location_address)) {
                $dynamic_post_location_address = $cs_xmlObject->dynamic_post_location_address;
            } else {
                $dynamic_post_location_address = '';
            }
            if (isset($cs_xmlObject->loc_city)) {
                $loc_city = $cs_xmlObject->loc_city;
            } else {
                $loc_city = '';
            }
            if (isset($cs_xmlObject->loc_postcode)) {
                $loc_postcode = $cs_xmlObject->loc_postcode;
            } else {
                $loc_postcode = '';
            }
            if (isset($cs_xmlObject->loc_region)) {
                $loc_region = $cs_xmlObject->loc_region;
            } else {
                $loc_region = '';
            }
            if (isset($cs_xmlObject->loc_country)) {
                $loc_country = $cs_xmlObject->loc_country;
            } else {
                $loc_country = '';
            }
            if (isset($cs_xmlObject->event_map_switch)) {
                $event_map_switch = $cs_xmlObject->event_map_switch;
            } else {
                $event_map_switch = '';
            }
            if (isset($cs_xmlObject->event_map_heading)) {
                ...
                <div class="option-sec" style="margin-bottom:0;">
                    <div class="opt-conts">
                        ...
                        <ul class="form-elements">
                            ...
                            <li class="to-field">
                                <input name="loc_city" data-zoom="<?php echo esc_attr($dynamic_post_location_zoom); ?>" id="loc_city" type="text" value="<?php echo esc_attr(htmlspecialchars($loc_city)); ?>" class="gllpSearchButton" onBlur="gll_search_map()" />
                            </li>
                        </ul>
                        <ul class="form-elements">
                            <li class="to-label">
...
if (!function_exists('cs_post_event_fields')) {
    function cs_post_event_fields() {
        ...
        if ($post_meta <> "") {
            ...
            if (isset($cs_xmlObject->dynamic_post_event_all_day)) {
                $dynamic_post_event_all_day = $cs_xmlObject->dynamic_post_event_all_day;
            } else {
        ...
        <script>
            jQuery(function () {
               var formatt= document.getElementById('cs_time_format').value;
                var str="24 hour ";  
               if (formatt != str)
               {
        ...
        <ul class="form-elements">
            ...
            <li class="to-field short-field">
                <input type="text" id="from_date" name="dynamic_post_event_from_date" value="<?php if (isset($dynamic_post_event_from_date) && $dynamic_post_event_from_date <> '')
            echo cs_allow_special_char($dynamic_post_event_from_date);
        else
            echo gmdate("Y/m/d");
        ?>" />
            </li>
            ...
            <li class="to-field">
                <div id="event_time" <?php /* ?><?php if($dynamic_post_event_all_day=='on')echo 'style="display:none"'?><?php */ ?>>
                    <div class="input-sec">
                        ...
                        $cs_time_format= $cs_theme_options['cs_time_formate'];
                        ?>
                        <input type="hidden" name="date_format" name="time_format" id="cs_time_format" value="<?php echo esc_html($cs_time_format);  ?> "/>
                        <input id="dynamic_post_event_start_time" name="dynamic_post_event_start_time" value="<?php
                               if (isset($cs_xmlObject->dynamic_post_event_start_time)) {
                                   echo esc_attr($cs_xmlObject->dynamic_post_event_start_time);
                               } else {
                                   echo date('H:i');
                               }
                    ...
                    <div class="input-sec">
                        <input id="dynamic_post_event_end_time" name="dynamic_post_event_end_time" value="<?php
                               if (isset($cs_xmlObject->dynamic_post_event_start_time)) {
                                   echo esc_attr($cs_xmlObject->dynamic_post_event_end_time);
                               } else {
                                   echo date('H:i');
                               }
                               ?>" type="text" class="vsmall"  />
                    ...
                    <div class="input-sec">
                        <div class="checkbox-list">
                            <div class="checkbox-item">
                                <input type="checkbox" name="dynamic_post_event_all_day" value="on" <?php
        ...
        <ul class="form-elements bcevent_title">
            ...
            <li class="to-field">
                <div class="input-sec">
                    <input type="text" id="dynamic_post_event_ticket_options" name="dynamic_post_event_ticket_options" value="<?php
            if (isset($cs_xmlObject->dynamic_post_event_ticket_options)) {
                echo esc_attr($cs_xmlObject->dynamic_post_event_ticket_options);
            }
            ?>" />
                    <label>Title</label>
                </div>
                <div class="input-sec">
                    <input type="text" id="dynamic_post_event_buy_now" name="dynamic_post_event_buy_now" value="<?php
            if (isset($cs_xmlObject->dynamic_post_event_buy_now)) {
                echo esc_attr($cs_xmlObject->dynamic_post_event_buy_now);
            }
            ?>" />
                    <label>Url</label>
                </div>
                <div class="input-sec">
                    <input type="text" name="dynamic_post_event_ticket_color" value="<?php
            if (isset($cs_xmlObject->dynamic_post_event_ticket_color)) {
                echo esc_attr($cs_xmlObject->dynamic_post_event_ticket_color);
            }
                    ?>" class="bg_color" />
                    <label>Colr</label>
                </div>
        ...
        <ul class="form-elements">
            ...
            <li class="to-field">
                <div class="input-sec">
                    <input type="text" id="dynamic_post_event_contact_no" name="dynamic_post_event_contact_no" value="<?php
                    if (isset($cs_xmlObject->dynamic_post_event_contact_no)) {
                        echo esc_attr($cs_xmlObject->dynamic_post_event_contact_no);
                    }
                    ?>" />
        ...
        <ul class="form-elements">
            ...
            <li class="to-field">
                <div class="input-sec">
                    <input type="text" id="dynamic_post_event_email" name="dynamic_post_event_email" value="<?php
                    if (isset($cs_xmlObject->dynamic_post_event_email)) {
                        echo esc_attr($cs_xmlObject->dynamic_post_event_email);
                    }
                    ?>" />
                </div>
            </li>
        ...
        <ul class="form-elements">
            ...
            <li class="to-field">
                <div class="input-sec">
                    <input type="text" id="dynamic_post_event_web_url" name="dynamic_post_event_web_url" value="<?php
                    if (isset($cs_xmlObject->dynamic_post_event_web_url)) {
                        echo esc_attr($cs_xmlObject->dynamic_post_event_web_url);
                    }
                    ?>" />
                </div>
            </li>
