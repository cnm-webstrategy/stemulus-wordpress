...
if (!function_exists('cs_events_register')) {
    ...
    function cs_events_register() {
        $labels = array(
            ...
            'search_items' => __('Search Event', 'Awaken'),
...
function cs_meta_event_add() {
    add_meta_box('cs_meta_event', __('Event Options', 'Awaken'), 'cs_meta_event', 'events', 'normal', 'high');
}
...
if (isset($_POST['event_meta_form']) and $_POST['event_meta_form'] == 1) {
    ...
    function cs_meta_event_save($post_id) {
        ...
        if (empty($_POST['dynamic_post_location_latitude'])) {
            $_POST['dynamic_post_location_latitude'] = '';
        }
        if (empty($_POST['dynamic_post_location_longitude'])) {
            $_POST['dynamic_post_location_longitude'] = '';
        }
        if (empty($_POST['dynamic_post_location_zoom'])) {
            $_POST['dynamic_post_location_zoom'] = '';
        }
        if (empty($_POST['dynamic_post_location_address'])) {
            $_POST['dynamic_post_location_address'] = '';
        }
        ...
        if (empty($_POST['event_map_switch'])) {
            $_POST['event_map_switch'] = '';
        }
        if (empty($_POST['event_map_heading'])) {
            $_POST['event_map_heading'] = '';
        }

        if (empty($_POST['dynamic_post_event_from_date'])) {
            $_POST['dynamic_post_event_from_date'] = "";
        }
        if (empty($_POST['dynamic_post_event_to_date'])) {
            $_POST['dynamic_post_event_to_date'] = "";
        }
        if (empty($_POST['dynamic_post_event_start_time'])) {
            $_POST['dynamic_post_event_start_time'] = "";
        }
        if (empty($_POST['dynamic_post_event_end_time'])) {
            $_POST['dynamic_post_event_end_time'] = "";
        }
        if (empty($_POST['dynamic_post_event_all_day'])) {
            $_POST['dynamic_post_event_all_day'] = "";
        }
        if (empty($_POST['dynamic_post_event_address'])) {
            $_POST['dynamic_post_event_address'] = "";
        }
        if (empty($_POST['dynamic_post_event_ticket_options'])) {
            $_POST['dynamic_post_event_ticket_options'] = "";
        }
        if (empty($_POST['dynamic_post_event_buy_now'])) {
            $_POST['dynamic_post_event_buy_now'] = "";
        }
        if (empty($_POST['dynamic_post_event_ticket_color'])) {
            $_POST['dynamic_post_event_ticket_color'] = "";
        }
        if (empty($_POST['dynamic_post_event_contact_no'])) {
            $_POST['dynamic_post_event_contact_no'] = "";
        }
        if (empty($_POST['dynamic_post_event_email'])) {
            $_POST['dynamic_post_event_email'] = "";
        }
        if (empty($_POST['dynamic_post_event_web_url'])) {
            $_POST['dynamic_post_event_web_url'] = "";
        }
        if (empty($_POST['dynamic_event_members'])) {
            $_POST['dynamic_event_members'] = '';
        }
        ...
        if (isset($_POST['dynamic_post_event_num_repeat']) && $_POST['dynamic_post_event_num_repeat'] != '') {
            ...
            for ($i = 1; $i < $_POST['dynamic_post_event_num_repeat']; $i++) {
                ...
                if (empty($_POST['dynamic_post_event_repeat'])) {
                    $_POST['dynamic_post_event_repeat'] = '';
                }
                ...
                $sxe = cs_page_options_save_xml($sxe);
                $from_date = get_post_meta($post_id, "dynamic_post_event_from_date", true);
                if ($count > 0) {
                    $from_date = $latest_val;
                }
                ...
                if (isset($from_date) && $from_date != '') {
                    ...
                    $cs_event_datetime = $from_date . ' ' . $_POST["dynamic_post_event_start_time"];
                    update_post_meta($inserted_id, 'cs_dynamic_event_from_date_time', strtotime($cs_event_datetime));
                }



                $event_from_date = date('Y/m/d', strtotime($from_date . $_POST["dynamic_post_event_repeat"]));
                $latest_val = $event_from_date;
                update_post_meta($inserted_id, "dynamic_post_event_from_date", $event_from_date);
                update_post_meta($inserted_id, 'events', $sxe->asXML());
                update_post_meta($inserted_id, 'dynamic_event_members', $event_organizer);
