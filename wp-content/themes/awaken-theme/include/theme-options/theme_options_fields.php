            ...
            switch ($value['type']) {
                ...
                case 'generate_backup':
                    ...
                    $output .= '
						<div class="cs-import-help">
							<h4>' . __('Import Options', 'Awaken') . '</h4>
						</div>';

                    $output .= '<div class="external_backup_areas">';

                    $output .= '<p>' . __('Input the URL from another location and hit Import Button to apply settings.', 'Awaken') . '</p>';

                    $output .= '<input type="text" id="bkup_import_url" />';
                    $output .= '<input id="cs-backup-url-restore" type="button" value="' . __('Import', 'Awaken') . '" />';

                    $output .= '</div>';

                    $output .= '
						<div class="cs-import-help">
							<h4>' . __('Export Options', 'Awaken') . '</h4>
						</div>';

                    if (is_array($cs_all_list) && sizeof($cs_all_list) > 0) {

                        $output .= '<p>' . __('Here you can Generate/Download Backups. Also you can use these Backups to Restore settings.', 'Awaken') . '</p>';
                        ...
                        if (isset($file_val['name'])) {
                            ...
                            $output .= '<input id="cs-backup-restore" data-file="' . $file_val['name'] . '" type="button" value="' . __('Restore', 'Awaken') . '" />';

                            $output .= '<a download="' . $file_val['name'] . '" href="' . esc_url($cs_upload_dir_path . $file_val['name']) . '">' . __('Download', 'Awaken') . '</a>';

                            $output .= '<input id="cs-backup-delte" data-file="' . $file_val['name'] . '" type="button" value="' . __('Delete', 'Awaken') . '" />';
                        }

                        $output .= '</div>';
                    }

                    $output .= '<input type="button" value="' . __('Generate Backup', 'Awaken') . '" onclick="javascript:cs_backup_generate(\'' . esc_js(admin_url('admin-ajax.php')) . '\');" />';
                ...
                case 'widgets_backup':
                    ...
                    if (class_exists('cs_widget_data')) {
                        ...
                        $output .= '
							<div class="cs-import-help">
								<h4>' . __('Import Widgets', 'Awaken') . '</h4>
							</div>';
                        ...
                        if (is_array($cs_all_list) && sizeof($cs_all_list) > 0) {
                            ...
                            if (isset($file_val['name'])) {
                                ...
                                $output .= '<input id="cs-wid-backup-restore" data-path="' . $cs_upload_dir_path . '" data-file="' . $file_val['name'] . '" type="button" value="' . __('Show Widget Settings', 'Awaken') . '" />';

                                $output .= '<a download="' . $file_val['name'] . '" href="' . esc_url($cs_upload_dir_path . $file_val['name']) . '">' . __('Download', 'Awaken') . '</a>';

                                $output .= '<input id="cs-wid-backup-delte" data-file="' . $file_val['name'] . '" type="button" value="' . __('Delete', 'Awaken') . '" />';
                            }
                        ...
                        $output .= '
							<div class="cs-import-help">
								<h4>' . __('Export Widgets', 'Awaken') . '</h4>
							</div>';
                    ...
                    $output .='<ul class="form-elements" id="' . $value['id'] . '_select">
								<li class="to-label"><label>' . esc_attr($value['name']) . '<span>' . esc_attr($value['hint_text']) . '</span></label></li>
									...
									<div class="input-sec">
										<div class="select-style">
										<select name="' . $value['id'] . '" id="' . $value['id'] . '">
										<option value="">' . __('Select Attribute', 'Awaken') . '</option>';
                ...
                case "upload":
                    ...
                    $output .= '<ul class="form-elements" id="' . $value['id'] . '_upload">
								  ...
								  <li class="to-field">
									...
									<input name="' . $value['id'] . '"  type="button" class="uploadMedia left" value="' . __('Browse', 'Awaken') . '"/></label>
								  </li>
								</ul>';
                    //if($val <> ''){			
                ...
                case "upload font":
                    ...
                    $output .= '<ul class="form-elements" id="' . $value['id'] . '_upload">
								  ...
								  <li class="to-field">
									...
									<label class="browse-icon">
										<input name="' . $value['id'] . '" type="button" class="uploadMedia left" value="' . __('Browse', 'Awaken') . '"/>
									</label>
								  </li>
								</ul>';
                ...
                case "upload favicon":
                    ...
                    $output .= '<label class="cs-browse"><input id="log" name="' . $value['id'] . '" type="button" class="uploadfile left" value="' . __('Browse', 'Awaken') . '"/><i class="fa fa-upload"></i></label></div></li></ul>';
                ...
                case "sidebar":
                    ...
                    $output .= '<ul class="form-elements">
									...
									<li class="to-field">
										...
										<input type="button" value="' . __('Add Sidebar', 'Awaken') . '" onclick="javascript:add_sidebar()" />
										<p>' . __('Please enter the desired title of sidebar', 'Awaken') . '</p>
									</li>
								</ul>
								<div class="clear"></div>
								<div class="sidebar-area" style="display:' . $display . '">
									<div class="theme-help">
									  <h4 style="padding-bottom:0px;">' . __('Already Added Sidebars', 'Awaken') . '</h4>
									  <div class="clear"></div>
									</div>
									<div class="boxes">
										...
										<thead>
											<tr>
												<th>' . __('Sider Bar Name', 'Awaken') . '</th>
												<th class="centr">' . __('Actions', 'Awaken') . '</th>
											</tr>
										</thead>
                ...
                case "networks":
                    ...
                    $output.='<ul class="form-elements">
								...
								<li class="to-field">
								  ...
								  <p>' . __('Please enter text for icon', 'Awaken') . '</p>
								</li>
								<li class="full">&nbsp;</li>
								<li class="to-label">
								...
								<div class="input-sec">
								  ...
								  <label class="browse-icon"><input id="social_net_icon_path_input" name="social_net_icon_path_input" type="button" class="uploadMedia left" value="' . __('Browse', 'Awaken') . '"/></label>
							   </li>
							  ...
							  <div class="theme-help">
								<h4 style="padding-bottom:0px;">' . __('Already Added Social Icons', 'Awaken') . '</h4>
								<div class="clear"></div>
							  </div>
							  <div class="boxes">
							  <table class="to-table" border="0" cellspacing="0">
								  <thead>
									<tr>
									  <th>' . __('Icon Path', 'Awaken') . '</th>
									  <th>' . __('Network Name', 'Awaken') . '</th>
									  <th>' . __('Url', 'Awaken') . '</th>
									  <th class="centr">' . __('Actions', 'Awaken') . '</th>
									</tr>
								  </thead>
                                ...
                                $output.='<tr id="' . str_replace(' ', '-', $social_net_tooltip[$i]) . '" style="display:none">
													  ...
													  <li><a onclick="cs_toggle(\'' . str_replace(' ', '-', $social_net_tooltip[$i]) . '\')"><img src="' . get_template_directory_uri() . '/include/assets/images/close-red.png" /></a></li>
														  ...
														  <li class="to-field">
															...
															<label class="browse-icon"><input id="social_net_icon_path' . $i . '" name="social_net_icon_path' . $i . '" type="button" class="uploadMedia left" value="' . __('Browse', 'Awaken') . '"/></label>
														  </li>
