...
if (!function_exists('cs_image_shortcode')) {
    ...
    function cs_image_shortcode($atts, $content = "") {
        ...
        $html .= '<div ' . $CustomId . ' class="image-frame ' . $image_style . ' ' . $cs_custom_animation . ' light-box gallery">';
        $html.= $section_title;
        if (isset($cs_image_url) && $cs_image_url != '') {

            $html .= '<a data-title="' . $cs_image_title . '"  href="' . esc_url($cs_image_url) . '" rel="prettyPhoto[gallery2]">';
            $html .= '<figure>';
            $html .= '<img src="' . esc_url($cs_image_url) . '" alt="" /></a>';
            $html .= '</figure>';
            $html .= '</a>';

            //$html	.= '<a data-title="'. $cs_image_title . '"  href="'.$cs_image_url.'" data-rel="prettyPhoto" rel="prettyPhoto">';
    ...
    function cs_gallery_shortcode($atts, $content = "") {
        ...
        if (!empty($cs_gal_album)) {
            ...
            if ($cs_gal_layout == 'gallery-slider') {
                ...
                $html .= '<div class="gallerylist js-isotope row light-box clearfix ' . $cs_gallery_class . ' ' . $cs_gallery_animation . '" style=" animation-duration: ' . $cs_custom_animation_duration . 's;" id="containerth">';
                if ($cs_meta_gallery_options <> "") {
                    for ($i = $limit_start; $i < $limit_end; $i++) {
                        $path = $cs_xmlObject->gallery[$i]->path;
                        $title = $cs_xmlObject->gallery[$i]->title;
                ...
                $html .= '<div class="gallerylist js-isotope row light-box clearfix ' . $cs_gallery_class . ' ' . $cs_gallery_animation . '" style=" animation-duration: ' . $cs_custom_animation_duration . 's;" id="containeron">';
